...
from docktape_lead_agent.delivery.slack import send_slack
from docktape_lead_agent.domain.results import ComplianceCheck, DeliveryRecord, FinalResult
from docktape_lead_agent.domain.scoring import calculate_fit
from docktape_lead_agent.domain.submissions import ComplianceOutcome, FinalStatus
from docktape_lead_agent.reporting.notification import build_notification, build_slack_blocks
from docktape_lead_agent.reporting.workbook import TRACKER_HEADERS, tracker_row, update_workbook
    ...
    return FinalResult(
        ...
        compliance_checks=[
            ComplianceCheck(
                ...
                outcome="clear",
                reason="Clear under exercise rules.",
            )
        ],
...
def test_workbook_updates_one_row_and_writes_untrusted_text_as_literal(
    tmp_path, lead, profile, assessment
) -> None:
...
def test_workbook_clears_stale_score_and_labels_synthetic_row(
    tmp_path, lead, profile, assessment
) -> None:
    result = make_result(lead, profile, assessment)
    path = tmp_path / "lead-tracker.xlsx"
    ...
    assert (
        sheet.cell(2, TRACKER_HEADERS.index("Company") + 1).value
        == "[SYNTHETIC TEST] Example Cloud"
    )
    workbook.close()
...
def test_workbook_migrates_legacy_headquarters_column(tmp_path, lead, profile, assessment) -> None:
    ...
    assert [cell.value for cell in sheet[1]] == TRACKER_HEADERS
    assert (
        "Example Cloud Inc: US (registered_jurisdiction)"
        in sheet.cell(2, TRACKER_HEADERS.index("Jurisdictions") + 1).value
    )
    workbook.close()
...
def test_notification_translates_competitor_flag_for_sales(lead, profile, assessment) -> None:
    ...
    sales_reason = (
        "This company offers cloud cost management services that directly compete with us."
    )

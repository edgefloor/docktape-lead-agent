...
from docktape_lead_agent.domain.acceptance import build_accepted_profile, render_summary
from docktape_lead_agent.domain.compliance import (
    evaluate_compliance,
    evaluate_country_compliance,
    route_status,
)
from docktape_lead_agent.domain.policy import load_policy
from docktape_lead_agent.domain.profiles import JurisdictionFact
...
from docktape_lead_agent.inference.profile import validate_profile_evidence
...
def test_jev_questions_are_choice_and_independent(lead, research, profile) -> None:
    ...
    assert "ada@example.com" not in str(request)
    assert request["state"]["evidence_index"][0]["id"] == research.evidence[0].id


def test_jev_rates_unlisted_competitor_overlap_on_an_ordered_score(lead, research, profile) -> None:
    policy = load_policy()
    question = build_jev_request(lead, research, profile, policy)["questions"][
...
def test_inconsistent_supported_profile_and_categories_cannot_score(
    ...
    parsed = parse_jev_response(
        _jev_payload(lead, research, profile, policy), profile, research, policy
    )
...
def test_compliance_flag_wins_and_summary_rejects_unsupported(
    assessment, profile, research, lead
) -> None:
    ...
    for key, question in request["questions"].items():
        ...
        if question["type"] == "score":
            answers[key] = {
                ...
                "score": 0.0,
                "probabilities": {
                    str(index): 1.0 if index == 0 else 0.0 for index in range(len(options))
                },
                "confidence": 0.99,
...
def test_malformed_jev_answer_becomes_validation_error_instead_of_aborting(
    lead, research, profile
) -> None:
    policy = load_policy()
    payload = _jev_payload(lead, research, profile, policy)
    ...
    assert (
        "missing or malformed Jev answer: competitor_evidence__spendwise_cloud"
        in parsed.validation_errors
    )
    assert parsed.competitor_evidence["spendwise_cloud"] in {
        "none",
        "submitted_name",
        "evidence_page",
    }
...
def test_dual_overlap_agreement_does_not_require_a_second_jev_claim_vote(
...
def test_score_uncertainty_within_non_competing_levels_still_clears(
    assessment, profile, research
) -> None:
    policy = load_policy()
...
def test_score_uncertainty_within_competing_levels_still_flags(
    assessment, profile, research
) -> None:
    policy = load_policy()
    profile.competitive_overlap.value = CompetitiveOverlap.COMPETING_SERVICE
...
def test_no_indication_competitor_is_clear_even_with_low_confidence(
    assessment, profile, research
) -> None:
    policy = load_policy()
...
def test_listed_country_flag_wins_over_conflict():
    policy = load_policy()
    facts = [
        JurisdictionFact(
    ...
    assert evaluate_country_compliance(facts, policy).outcome == ComplianceOutcome.FLAGGED


def test_direct_identity_needs_page_evidence(research, profile):
    research.evidence[0].source_type = "search_snippet"
    with pytest.raises(AssessmentError, match="page evidence"):
        validate_profile_evidence(profile, research)

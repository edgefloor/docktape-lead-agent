...
from docktape_lead_agent.domain.judgments import ChoiceAnswer, ScoreAnswer
from docktape_lead_agent.domain.submissions import LeadSubmission


def test_stable_id_ignores_scheme_www_and_url_details() -> None:
    first = LeadSubmission(
        name="A",
        email="USER@Example.com",
        company_name="  Acme   Cloud ",
        website="https://www.acme.com/a?q=1",
        employee_count_band="unknown",
    )
    second = LeadSubmission(
        name="B",
        email="user@example.com",
        company_name="acme cloud",
        website="http://acme.com/other#x",
        employee_count_band="unknown",
    )
...
def test_employee_band_is_required() -> None:
    with pytest.raises(ValidationError):
        LeadSubmission(
            name="A", email="a@example.com", company_name="Acme", website="https://acme.com"
        )
...
def test_choice_answer_rejects_invalid_probability_distributions(
...
def test_choice_answer_requires_selected_highest_probability() -> None:
    with pytest.raises(ValidationError):
        ChoiceAnswer(
            type="choice", choice="no", probabilities={"yes": 0.8, "no": 0.2}, confidence=0.9
        )
